...
if test -x PerlMagick -a -f Makefile.aperl ; then
  ...
  ${MAKE} -f Makefile.aperl CC='/home/wrv/CircleSquareFuzzing/afl-1.85b/afl-gcc -std=gnu99 -std=gnu99' TEST_VERBOSE=1 test
elif test -f Makefile -a -f Magick.o; then
  # Shared build test incantation
  ${MAKE} CC='/home/wrv/CircleSquareFuzzing/afl-1.85b/afl-gcc -std=gnu99 -std=gnu99' TEST_VERBOSE=1 test
else
